...
They don't know each other's x'<sub>w</sub>, but they can compute the corresponding X'<sub>w</sub> from the public X<sub>w</sub>. The sum of X'<sub>A</sub> and X'<sub>B</sub> will be called the temporal common public key, which I'll write with JK:
